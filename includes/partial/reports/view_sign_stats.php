<?php
	...
	$STH = $DBH->Prepare("SELECT closeengineerid, Count(callid) as count FROM calls WHERE closed >= DATE_SUB(CURDATE(),INTERVAL 7 DAY) AND status=2 GROUP BY closeengineerid ORDER BY count DESC LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$STH->execute();
	while($row = $STH->fetch()) {

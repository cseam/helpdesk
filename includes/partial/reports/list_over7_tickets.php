...
<div id="ajaxforms">
	<?php
		if ($_SESSION['engineerHelpdesk'] <= '3') {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk <= :helpdeskid AND status !='2' AND DATE(calls.opened) <= DATE_SUB(CURDATE(),INTERVAL 7 DAY) ORDER BY callID");
			$hdid = 3;
		} else {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk = :helpdeskid AND status !='2' AND DATE(calls.opened) <= DATE_SUB(CURDATE(),INTERVAL 7 DAY) ORDER BY callID");
		...
		$STH->execute();
		echo ("<h3>". $STH->rowCount() ." - Tickets older than 7 days</h3><table><tbody>");
		if ($STH->rowCount() == 0) { echo "<p>No Tickets open older than 7 days</p>";};
		while($row = $STH->fetch()) {
		?>

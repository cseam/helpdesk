...
<div id="ajaxforms">
	<?php
		if ($_SESSION['engineerHelpdesk'] <= '3') {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk <= :helpdeskid AND status ='3' ORDER BY callID DESC");
			$hdid = 3;
		} else {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk = :helpdeskid AND status ='3' ORDER BY callID DESC");
		...
		$STH->execute();
		echo ("<h3>". $STH->rowCount() ." - Tickets on Hold </h3><table><tbody>");
		if ($STH->rowCount() == 0) { echo "<p>No Tickets on Hold</p>";};
		while($row = $STH->fetch()) {
		?>

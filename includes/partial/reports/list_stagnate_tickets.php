...
<div id="ajaxforms">
	<?php
		if ($_SESSION['engineerHelpdesk'] <= '3') {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk <= :helpdeskid AND status IN (1,4) AND DATE(calls.lastupdate) <= DATE_SUB(CURDATE(),INTERVAL 72 HOUR) ORDER BY callID");
			$hdid = 3;
		} else {
			$STH = $DBH->Prepare("SELECT * FROM calls INNER JOIN status ON calls.status=status.id INNER JOIN location ON calls.location=location.id WHERE helpdesk = :helpdeskid AND status IN (1,4) AND DATE(calls.lastupdate) <= DATE_SUB(CURDATE(),INTERVAL 72 HOUR) ORDER BY callID");
		...
		$STH->execute();
		echo ("<h3>". $STH->rowCount() ." - Stagnate Tickets</h3><p>Tickets with no update in last 72hours</p><table><tbody>");
		if ($STH->rowCount() == 0) { echo "<p>0 Tickets</p>";};
		while($row = $STH->fetch()) {
		?>
